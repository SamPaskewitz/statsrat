...
Finally, *statsrat* assumes that all model processing is deterministic: the only possibly stochastic part of the simulation is behavior.  This is an essential part of the package's design: it ensures that each model's likelihood function is well-defined for any sequence of behavioral and environmental data.  Without this, parameter estimation is not possible (or at any rate would be extremely troublesome).  For example, consider the instance-based learning model MINERVA-2 *** REF *** .  MINERVA-2 decides stochastically whether or not to store training instances in memory, and therefore cannot be implemented in *statsrat* as originally described.  However one could implement a modified version of MINERVA-2 which - rather than stochastingly deciding to store training instances with probability *1 - 1/n* - instead declines to store every *n*th training instance.  With appropriately randomized trials this ought to produce similar behavior. *** CHECK MATH ***
...
* deep learning *** REF ***
* models with stochastic processing (unless modified) *** REFS ***
