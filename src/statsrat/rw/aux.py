...
def Kalman(state, n, env, sim_pars, purpose): # FINISH FIXING
    '''
    Kalman filter Rescorla-Wagner (Dayan & Kakade 2001, Gershman & Diedrichsen 2015).
    '''
    if purpose == 'initialize':
        new_state = {'gain': np.zeros([n['f'], n['y']]), 'Sigma': np.ones(n['f'])}
        new_state_dims = {'f_counts': ['f_name'], 'atn': ['f_name']}
        new_state_sizes = {'f_counts': [n['f']], 'atn': [n['f']]}
        return new_state, new_state_dims, new_state_sizes
    ...
    def __init__(self, state, n, env, sim_pars):
        self.data = {'gain' : np.zeros((n['t'] + 1, n['f'], n['y'])), 'Sigma' : np.zeros((n['t'] + 1, n['y'], n['f'], n['f']))}
        for i in range(n['y']):
            self.data['Sigma'][0, i, :, :] = sim_pars['w_var0']*np.identity(n['f']) # initialize weight covariance matrix
    
    def update(self, state, n, env, sim_pars):
        for i in range(n['y']):
            f = state['fbase'].reshape((n['f'], 1))
            drift_mat = sim_pars['drift_var']*np.identity(n['f'])
            ...
            self.data['Sigma'][t + 1, i, :, :] += drift_mat - self.data['gain'][t, :, i].reshape((n['f'], 1))@f.transpose()@(self.data['Sigma'][t, i, :, :] + drift_mat) # update weight covariance matrix

    def add_data(self, ds):
        # possibly add Sigma and the diagonal of Sigma later
        return ds.assign(gain = (['t', 'f_name', 'y_name'], self.data['gain'][range(n['t']), :, :]))
