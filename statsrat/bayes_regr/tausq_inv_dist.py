...
class ard:
    ...
    def __init__(self, n_u, n_f, sim_pars):
        self.n_u = n_u
        self.hpar0_tausq_inv = np.array(n_f*n_u*[sim_pars['prior_tausq_inv_hpar0']], dtype='float').reshape((n_f, n_u))
        self.hpar1_tausq_inv = sim_pars['prior_tausq_inv_hpar1']
    ...
    def update(self, mean_wsq, u_psb):
        ...
        for j in range(self.n_u):
            self.hpar0_tausq_inv[:, j] += -0.5*mean_wsq[:, j]
        self.hpar1_tausq_inv += 0.5*self.u_psb_so_far.sum()
    
    def mean_tausq_inv(self):
        return (self.hpar1_tausq_inv + 1)/(-self.hpar0_tausq_inv)
    
    def mean_tausq(self):
        return -self.hpar0_tausq_inv/self.hpar1_tausq_inv
...
class ard_drv_atn:
    ...
    def __init__(self, n_u, n_f, sim_pars):
        self.n_u = n_u
        self.n_f = n_f
        self.hpar0_tausq_inv = np.array(n_f*[sim_pars['prior_tausq_inv_hpar0']], dtype='float')
        self.hpar1_tausq_inv = sim_pars['prior_tausq_inv_hpar1']
        self.u_psb_so_far = np.zeros(n_u)
    ...
    def update(self, mean_wsq, u_psb):
        ...
        for j in range(self.n_u):
            if u_psb[j] == 1:
                self.u_psb_so_far[j] = 1
        # update hyperparameters
        self.hpar0_tausq_inv += -0.5*mean_wsq.sum(1)
        self.hpar1_tausq_inv += 0.5*self.u_psb_so_far.sum()
    
    def mean_tausq_inv(self):
        mean_tausq_inv = np.zeros((self.n_f, self.n_u))
        for i in range(self.n_f):
            mean_tausq_inv[i, :] = (self.hpar1_tausq_inv + 1)/(-self.hpar0_tausq_inv[i])
        return mean_tausq_inv
    
    def mean_tausq(self):
        mean_tausq = np.zeros((self.n_f, self.n_u))
        for i in range(self.n_f):
            mean_tausq[i, :] = -self.hpar0_tausq_inv[i]/self.hpar1_tausq_inv
        return mean_tausq
